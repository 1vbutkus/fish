    ...
    def update_from_ws_message(self, message: dict):
        ...
        if event_type == 'book':
            ...
            for change in message['changes']:
                self.lob1000.update_single(price=float(change['price']), size=float(change['size']), side=change['side'])
...
def __dummy__():
    ...
    clob_public_mob_0_cache = PublicMarketBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
    ...
    clob_public_mob_1_cache = PublicMarketBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
    for clob_mob in clob_mob_list:
    ...
    ws_public_mob_1_cache = PublicMarketBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
