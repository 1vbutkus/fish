...
class HouseOrderBook(BaseMarketOrderBook):
    ...
    def __eq__(self, other):
        if not isinstance(other, HouseOrderBook):
            return False
        return self.condition_id == other.condition_id and self.yes_asset_book == other.yes_asset_book and self.no_asset_book == other.no_asset_book
    ...
    def update_from_clob_house_order_list(self, clob_house_order_list: list[dict]):
        ...
        yes_asset_book, no_asset_book = self.get_asset_books_from_live_orders(live_order_dict=live_order_dict)
        self.yes_asset_book, self.no_asset_book, self.house_live_order_dict = yes_asset_book, no_asset_book, live_order_dict
    ...
    def update_from_ws_messages(self, ws_message_list: list[dict]):
        ...
        self.yes_asset_book, self.no_asset_book = self.get_asset_books_from_live_orders(live_order_dict=self.house_live_order_dict)
...
def __dummy__():
    ...
    clob_house_order_book_cache = HouseOrderBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
    clob_house_order_book_cache.update_from_clob_house_order_list(clob_house_order_list=clob_house_order_list)

    clob_house_order_list = info_step_list[2]['clob_house_order_list']
    self = clob_house_order_book_cache = HouseOrderBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
    clob_house_order_book_cache.update_from_clob_house_order_list(clob_house_order_list=clob_house_order_list)

    #
    ws_house_order_book_cache = HouseOrderBook.new_from_clob_market_info_dict(clob_market_info_dict=clob_market_info_dict)
    clob_house_order_list = info_step_list[1]['clob_house_order_list']
    ws_house_message_list = info_step_list[2]['ws_house_message_list']
    ws_house_order_book_cache.update_from_clob_house_order_list(clob_house_order_list=clob_house_order_list)
