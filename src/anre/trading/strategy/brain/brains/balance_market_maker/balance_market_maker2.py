...
class BalanceMarketMaker(StrategyBrain):
    ...
    def update_state_and_get_action_list(self, action_freeze: bool) -> list[StrategyAction]:
        ...
        keep_long_price1000_range = [
            target_long_price1000 - config.keep_offset_down_level * step1000,
            target_long_price1000 + config.keep_offset_up_level * step1000,
        ]
        keep_short_price1000_range = [
            target_short_price1000 - config.keep_offset_down_level * step1000,
            target_short_price1000 + config.keep_offset_up_level * step1000,
        ]
        ...
        for order_dict in monitor.get_house_order_dict_list():
            ...
            if order_dict['bool_side'] == 'LONG':
                ...
                raise ValueError(f'Unknown bool_side {order_dict["bool_side"]}')
