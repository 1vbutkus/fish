...
class Parameter(ParameterBase):
    ...
    def __init__(self, *params, strategyLabel: Optional[str] = None, skipValidation=False):
        ...
        if strategyLabel:
            ...
            _extraPath = anre_config.path.get_path_to_resource_dir(
                'strategyPredefined', f'{strategyLabel}.yaml'
            )
        ...
        if not skipValidation:
            assert self._validate(
                paramDotDict=self.paramDotDict, paramDerivedDotDict=self.paramDerivedDotDict
            )
            if strategyLabel:
                assert self.paramDotDict['strategyLabel'] == strategyLabel, (
                    f'Strategy labels is not matching: {self.paramDotDict["strategyLabel"]} vs {strategyLabel}'
                )

    @classmethod
    def _validate(cls, paramDotDict, paramDerivedDotDict) -> bool:
        paramsDict = DotNest.convert_dotDict2nest(paramDotDict)
        assert set(paramsDict) == {'strategyLabel', 'configDict', 'tagDict', 'comment'}, (
            f"params labels must be ['strategyLabel', 'configDict', 'tagDict', 'comment'], but got: {set(paramDotDict)}"
        )
